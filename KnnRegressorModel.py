...
# Apply the same scaling learned from the training data to the test data
...
best_k_A = np.argmin(mae_A_list) + 1 # Finds the index of the lowest error in the list (add 1 because lists start at 0)
...
train_pred_A = final_model_A.predict(X_train_A_scaled)

# Evaluates both testing and training set to check if overfitted or not
print("Training MAE:", mean_absolute_error(y_train_A, train_pred_A))
print("Testing MAE:", mean_absolute_error(y_test_A, pred_A))
